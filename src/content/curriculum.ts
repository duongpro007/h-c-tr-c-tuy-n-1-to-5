...
// Đánh dấu toàn bộ chủ đề lõi trong danh sách là "ready" (đã có bài giảng + quiz đầy đủ),
// dùng khi một khối lớp/môn đã được biên soạn xong nội dung chi tiết trong lessons.ts.
function readyAll(topics: Topic[]): Topic[] {
  return topics.map((t) => ({ ...t, opts: { ...t.opts, ready: true } }));
}
